...
<xsl:template match="/instructionsection">
    ...
    <xsl:apply-templates select="/instructionsection/aliastablehook"/>
    ...
    <xsl:if test="/instructionsection/aliasto and not(/instructionsection/ps_section)">
      ...
      <div class="alias_ps_section">
        ...
        <xsl:apply-templates select="/instructionsection/aliasto"/> 
        gives the operational pseudocode for this instruction.</p>
      </div>
    </xsl:if>
    ...
    <!-- any notes on the operation of the instruction, e.g. fixed timing -->
    <xsl:apply-templates select="/instructionsection/operationalnotes"/>

    <xsl:if test="/instructionsection/desc/uses_dit='True' or /instructionsection/desc/takes_movprfx='True'">
  ...
  <xsl:template name="generate_footer">
    ...
    <!-- version footer -->
    <p class="versions">
      Internal version only: isa v01_19, pseudocode v2020-09_xml, sve v2020-09_rc3
      ; Build timestamp: 2020-09-30T21:35
    </p>
    <p class="copyconf">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <p class="desc">
          ...
          <p><xsl:apply-templates select="./intrinsics"/></p>
...
<!-- 
 ...
 -->
  ...
  <xsl:template match="aliastablehook">
    <xsl:if test="//alias_list/aliasref[./aliaspref/text() != 'Never']">
      <a name="{@anchor}"></a>
      <h3 class="aliastable"><xsl:value-of select="."/></h3>
