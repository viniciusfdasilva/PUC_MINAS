...
public class Parse {
	...
	void casaToken(byte token){
	 ...
	 void S(){
		 try{
			 ...
			 if(s != null){
				b.buffer.add("sseg SEGMENT STACK ;inicio seg. pilha");
				b.buffer.add("byte 4000h DUP(?) ;dimensiona pilha");
				b.buffer.add("sseg ENDS ;fim seg. pilha");
				b.buffer.add("dseg SEGMENT PUBLIC ;inicio seg. dados");
				b.buffer.add("byte 4000h DUP(?) ;temporarios");
				endereco = memoria.alocarTemp();
		 		...
		 		b.buffer.add("cseg SEGMENT PUBLIC ;inicio seg. codigo");
				...
				b.buffer.add("cseg ENDS ;fim seg. codigo");
	...
	String tipo() throws Exception{
		if(s.getToken() == tabela.INT){
			...
			System.err.println(lexico.linha + ":Token nao esperado.");
			System.exit(0);
			return null;
		}
	}
	...
	void C() throws Exception{
		...
		if(s.getToken() == tabela.ID){
			...
			if(s.getClasse().equals("classe_const")){
				//erro
				System.err.println(lexico.linha + ":classe de identificador incompativel ["+s.getLexema()+"]");
				System.exit(0);
			}
			tmp = s;
			casaToken(tabela.ID);
