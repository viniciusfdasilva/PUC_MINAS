  ...
  */
  public void merge(int indiceInicio, int meio, int indiceFim){
    ...
    while(i <= meio && j <= indiceFim){
      ...
      if(auxiliar[i].getNumeroEpisodios() < auxiliar[j].getNumeroEpisodios() || auxiliar[i].getNumeroEpisodios() == auxiliar[j].getNumeroEpisodios() && auxiliar[i].getNome().compareTo(auxiliar[j].getNome()) < 0){
        contNumeroMovimentacoes();
        array[k] = auxiliar[i];
        i++;
      }else{
...
public class Main{
  ...
  public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
    FileWriter file = new FileWriter("matrícula_mergesort");
