   ...
   public void selecao(){
      for(int i = 0; i < (n-1); i++) {
        ...
        for(int j = (i+1); j < n; j++){
          ...
          if(array[menor].getPaisOrigem().compareTo(array[j].getPaisOrigem()) == 0){
            contNumeroComparacoes();
            if(array[menor].getNome().compareTo(array[j].getNome()) > 0){
              menor = j;
            }// End if
          }// End if
          else if(array[menor].getPaisOrigem().compareTo(array[j].getPaisOrigem()) > 0){
            contNumeroComparacoes();
             menor = j;              
          }// End else if
          else{contNumeroComparacoes();}
        }// End for
  ...
  /**
    ...
    */
	public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
		FileWriter file = new FileWriter("matrícula_selecao");
