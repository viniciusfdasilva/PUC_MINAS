...
/**
  ...
  public int getMaior(){
     selecao();
     int maior = array[0].getNumeroTemporadas();
    for (int i = 0; i < n; i++){
        contNumeroComparacoes();
         if(maior < array[i].getNumeroTemporadas()){
            maior = array[i].getNumeroTemporadas();
         }// End if
    }// End for
     return maior;  
  }// End getMaior
  ...
  public void countingsort(){
    ...
    for(int i = 0; i < count.length; count[i] = 0, i++,contNumeroComparacoes());
    //Agora, o count[i] contem o numero de elemento iguais a i
    for(int i = 0; i < n; count[array[i].getNumeroTemporadas()]++, i++);
      //Agora, o count[i] contem o numero de elemento menores ou iguais a i
      for(int i = 1; i < count.length; count[i] += count[i-1], i++);
      //Ordenando
      for(int i = n-1; i >= 0; ordenado[count[array[i].getNumeroTemporadas()]-1] = array[i], count[array[i].getNumeroTemporadas()]--, i--,contNumeroMovimentacoes());
...
public class Main{
  ...
  public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
    FileWriter file = new FileWriter("matrícula_countingsort");
