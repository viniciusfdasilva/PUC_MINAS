...
class Lista{
    ...
    */
   public void insercaoPorCorNome(int cor, int h){
  ...
  /**
    ...
    * @param int h passo do shelsort
    */
   public void insercaoPorCorIdioma(int cor, int h){
      for(int i = (h+cor); i < n; i+=h){
         Serie tmp = array[i];
         int j = i - h; 
        	while((j >= 0) && (array[j].getIdiomaOriginal().compareTo(tmp.getIdiomaOriginal()) >= 1)){
        	    contNumeroComparacoes();
              contNumeroMovimentacoes();
              array[j+h] = array[j];
        	    j-=h;
           ...
           contNumeroComparacoes();
        	while((j >= 0) && (array[j].getNome().compareTo(tmp.getNome()) >= 1) && array[j].getIdiomaOriginal().equals(tmp.getIdiomaOriginal())){
        	    contNumeroComparacoes();
              contNumeroMovimentacoes();
              array[j+h] = array[j];
        	    j-=h;
  ...
  /**
    ...
    */
	public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
		FileWriter file = new FileWriter("matrícula_shellsort");
