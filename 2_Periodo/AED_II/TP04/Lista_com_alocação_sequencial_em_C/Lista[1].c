 ...
 * do índice
 */	
char* delete(char* str,int i, int j){
	int c = 0;
 ...
 * @return int
 */	
int indexOff(char* str,char chr){
	int i = 0;
	while(str[i] != chr && i < (int)strlen(str)) i++;
...
char* removeTag(char* str){
	int i = 0;
	int j = 0;
	for(i = indexOff(str,'<'),j = indexOff(str,'>'); i >= 0 && j >= 0; i = indexOff(str,'<'),j = indexOff(str,'>')){
		str = delete(str,i,j);		
	}// End for
	return str;
}// End removeTag()
