...
class Lista{
   ...
   /**
   * Recebe um ponteiro serie
   * e retorna um valor doidão.
   *  @param Serie
    * @return int
   */
   public int getDoidao(Serie serie){
      return ((serie.getNumeroEpisodios() * 1000) + serie.getNumeroTemporadas());
   }// End getDoidao()
  ...
  /**
   * Algoritmo de ordenacao do RadixSort.
   */
   ...
   */
  public int getMaior(){
     int maior = getDoidao(array[0]);
    for(int i = 0; i < n; i++){
         contNumeroMovimentacoes();
         if(maior < getDoidao(array[i])){
            maior = getDoidao(array[i]);
         }// End if
    }// End for
     return maior;  
  }// End getMaior
  ...
  public void countingsort(int exp){
    ...
    for(int i = 0; i < n; count[(getDoidao(array[i])/exp)%10]++, i++);
      //Agora, o count[i] contem o numero de elemento menores ou iguais a i
      for(int i = 1; i < count.length; count[i] += count[i-1],contNumeroMovimentacoes(),i++);
      //Ordenando
      for(int i = n-1; i >= 0; ordenado[count[(getDoidao(array[i])/exp)%10]-1] = array[i],contNumeroMovimentacoes(),count[(getDoidao(array[i])/exp)%10]--, i--);
      //Copiando para o array original
      for(int i = 0; i < n; array[i] = ordenado[i],contNumeroMovimentacoes(),i++);
   }// End contingsort()
...
public class Main{
  ...
  public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
    FileWriter file = new FileWriter("matrícula_radixsort");
