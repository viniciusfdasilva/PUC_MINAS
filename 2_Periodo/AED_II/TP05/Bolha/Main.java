  ...
  public void bolha(){
    for(int i = (n - 1); i > 0; i--){
      for(int j = 0; j < i; j++){
        ...
        if(array[j].getNumeroTemporadas() == array[j+1].getNumeroTemporadas()){
          contNumeroComparacoes();         
          if(array[j].getNome().compareTo(array[j+1].getNome()) >= 1){
            contNumeroMovimentacoes();
            swap(j,j+1);
          }// End if
        }// End if
        else if(array[j].getNumeroTemporadas() > array[j+1].getNumeroTemporadas()){
          contNumeroComparacoes();
          contNumeroMovimentacoes();
          swap(j, j+1);
        }// End else if
...
public class Main{
  ...
  public static void createFile(int matricula,int comparacoes,int movimentacoes,double tempo) throws Exception{
    FileWriter file = new FileWriter("matrícula_bolha");
